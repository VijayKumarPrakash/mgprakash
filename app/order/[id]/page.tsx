  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12 space-y-8">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center justify-center w-14 h-14 rounded-full mb-2" style={{ background: 'color-mix(in srgb, var(--accent) 12%, transparent)' }}>
        ...
        <p className="text-[var(--ink-3)]">
          Thank you, <strong>{o.client_name}</strong>. We&rsquo;ve received your catering request and will be in touch shortly to confirm availability and pricing. A summary has been sent to <strong>{o.client_email}</strong>.
        </p>
        ...
        <a
          ...
          download
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[var(--line)] text-sm font-medium text-[var(--ink-2)] hover:border-[var(--line-strong)] hover:bg-[var(--surface-2)] transition-colors bg-[var(--surface)] mt-2"
        >
      ...
      <section className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-6 space-y-4">
        ...
        <div>
          <p className="font-semibold text-lg text-[var(--ink)]">{o.event_name}</p>
          <p className="text-[var(--ink-3)] capitalize text-sm">{o.event_type}</p>
        </div>
        <div className="pt-2 border-t border-[var(--line)] text-sm text-[var(--ink-2)] space-y-1">
          <p><span className="text-[var(--ink-3)]">Client: </span>{o.client_name}</p>
          <p><span className="text-[var(--ink-3)]">Email: </span>{o.client_email}</p>
          <p><span className="text-[var(--ink-3)]">Phone: </span>{o.client_phone}</p>
        </div>
      </section>
      ...
      {mealsWithDishes.map((meal, i) => (
        <section key={meal.id} className="bg-[var(--surface)] border border-[var(--line)] rounded-2xl p-6 space-y-4">
          <div>
            <p className="text-xs font-semibold text-[var(--ink-3)] uppercase tracking-widest mb-1">Meal {i + 1}</p>
      ...
      <div className="text-center pt-4 space-y-3">
        ...
        <Link href="/" className="inline-flex items-center text-sm font-medium hover:opacity-80 transition-opacity" style={{ color: 'var(--accent)' }}>
          ← Return to home
        </Link>
      </div>
    </div>
